    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-on-surface mb-4">Project not found</h2>
    ...
    <div className="bg-surface font-body text-on-surface antialiased min-h-screen">
      ...
      <main className="pt-24 pb-32 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-10">
          <div className="flex items-center gap-2 text-sm font-semibold tracking-wide text-on-surface-variant uppercase">
          ...
          <div className="flex items-center gap-4 bg-surface-container-low px-4 py-2 rounded-xl">
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${project.status === 'Ongoing' ? 'bg-orange-500 animate-pulse' : 'bg-green-500'}`}></span>
              <span className="text-xs font-bold uppercase tracking-widest text-on-surface">{project.status}</span>
            </div>
            <div className="h-4 w-[1px] bg-outline-variant/30"></div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-primary">{project.progress}% Complete</span>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-20 items-center">
          <div className="lg:col-span-5">
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {project.tags && project.tags.map((tag, i) => (
                <span key={i} className="px-3 py-1.5 bg-secondary-container text-on-secondary-container rounded-lg text-xs font-bold uppercase tracking-widest flex items-center gap-1">
                  <span className="material-symbols-outlined text-[14px]">label</span> {tag}
                </span>
              ))}
            </div>
            <button className="primary-gradient text-white px-6 py-3 rounded-lg font-bold flex items-center gap-2 shadow-lg hover:scale-[1.02] transition-transform w-fit" onClick={() => alert("Downloading Project Brief...")}>
          ...
          <div className="lg:col-span-7 relative">
            {project.youtubeLink ? (
              <div className="aspect-video rounded-2xl overflow-hidden shadow-2xl relative bg-black border-[8px] border-surface-container-lowest">
                <iframe 
              ...
              <div className="aspect-video rounded-2xl overflow-hidden bg-surface-container-low shadow-2xl relative border-[8px] border-surface-container-lowest">
        ...
        {/* Project Mentor Section - Full width */}
        {mentor && (
        ...
        {/* Team Members Section - Full width, below mentor */}
        ...
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {project.gallery && project.gallery.map((img, i) => (
              <div key={i} className="aspect-square rounded-2xl overflow-hidden bg-surface-container-lowest group relative shadow-md hover:shadow-lg transition-all border border-outline-variant/10">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-5 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end h-1/2">
                  <span className="text-white font-bold text-lg tracking-tight truncate">{img.title}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {project.methodology && project.methodology.length > 0 && (
          <section className="bg-surface-container-lowest rounded-3xl p-8 md:p-12 border border-outline-variant/20 shadow-sm relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
              <div className="space-y-8">
                {project.methodology.map((method) => (
                  <div key={method.id} className="flex gap-5">
                    <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-primary-container text-primary flex items-center justify-center font-bold text-lg shadow-sm">{method.id}</div>
                    <div>
                      <h4 className="font-bold text-lg text-on-surface mb-2">{method.title}</h4>
              ...
              {project.milestone && (
                <div className="bg-gradient-to-br from-secondary-container/50 to-surface-container p-8 rounded-2xl border border-secondary/20 self-start">
                  ...
                  <button className="text-white bg-secondary px-6 py-3 rounded-lg font-bold text-sm hover:opacity-90 transition-opacity w-full flex justify-center items-center gap-2" onClick={handleFeatureClick}>
